...
exports.findloggedUser = (emailORphone, password) => {
    return new Promise((resolve, reject) => {
        connection.query(
            "SELECT * FROM customers WHERE (phone = ? OR email = ?) AND password = ?", [emailORphone, emailORphone, password]
        , (err, result) => {
            if(err){
                reject(err);
            }else{
...
exports.addAddress = (address, cust_id) => {
    ...
    return new Promise((resolve, reject)=> {
        connection.query(
            ...
            [cust_id, address.street, address.city, address.state, address.country, address.postalcode, address.houseNo],
            (err, result) => {
                if(err){
                    reject(err);
                }else{
