...
// reducing the quantity of items on order status sucess.
exports.reduceProductQuantity = (
  product_id,
  product_quantity,
  cart_item_quantity
) => {
  return new Promise((resolve, reject) => {
    connection.query(
      ...
      [Number(product_quantity) - Number(cart_item_quantity), product_id],
      (err, result) => {
        if (err) {
          reject(err);
        } else {
...
// removing the purchased items from the user cart
exports.removeOrderedItems = (cart_item_id, cart_id) => {
  return new Promise((resolve, reject) => {
    connection.query(
      "DELETE FROM cart_items where cart_item_id = ? && cart_id = ?",
